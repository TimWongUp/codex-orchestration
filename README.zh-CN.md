...
- **Review 是独立交付门。** `codex-review-gate` 独立定义路由，根主代理执行分级和整改：R0 只覆盖可完整验证的非行为改动；R1 对一个局部、已验证、可恢复风险使用一名 Reviewer，局部公共合同或托管策略改动也可属于 R1；R1 默认使用 `correctness-reviewer`，唯一风险明确属于专项时由匹配的专项 Reviewer 替代而非叠加；R2 覆盖多个独立风险、广泛或难恢复的公共合同、敏感边界及其他未归类风险；R3 对信任边界变化或高影响失败追加专项整改与对抗式复核。R1–R3 始终至少使用一名匹配 Reviewer，只有新增实质失败假设才增加席位，不设人数配额。
