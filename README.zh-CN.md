...
Codex Orchestration 刻意不追求“Agent 越多越好”。简单任务仍由主代理直接完成；只读 Agent 可以按证据范围或独立观点并行调查，每个根任务内部则遵循本地单 Writer 租约。当用户明确要求使用 Codex 官方 Worktree，且任务可以安全拆分时，一个 Integration Root 最多可协调三个独立 Worktree Root 并行实现。每个 Worktree Root 都在分配的工作线内按普通根任务运行；全部声明工作线通过交接验收前，Integration Root 及其本地 Worker 均不得写仓库，之后它才串行合并整批结果并负责组合验证。处理集成 PR 时进行风险 Review；获准合并主分支时确认已有审查仍覆盖最终候选。
...
- **委派有门槛。** 只有并行证据、专业分工或边界清晰的 Worker 能实质改善结果时，才启动子代理；独立的 PR Review 策略只授权它选中的只读 Reviewer。
...
- **PR 阶段审查，合并前确认覆盖。** 创建或更新 PR、受托审查现有 PR（含外部贡献），以及获准合并主分支前，加载 `codex-review-gate`。Review 不依赖 CI；有相关 CI 时并行推进，无 CI 时完成适用的本地验证。普通本地交付和仅分支交接不触发。R0 由主代理验证，R1 选择一名匹配 Reviewer，R2 覆盖更广或不确定的风险，R3 追加对抗复核。合并前复用覆盖最新候选的有效证据，仅补审受影响风险；修改贡献者分支和合并仍需对应授权。这些规则不会安装 GitHub 事件监听。
