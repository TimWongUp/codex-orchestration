...
Keep every requested agent in the pending set until that target's own `wait_agent` status entry is `completed`, `errored`, `interrupted`, `shutdown`, or `not_found`, and consolidate only after the pending set is empty. `timed_out=true`, an empty or missing status map, an omitted target entry, and an unrecognized status are non-terminal; keep those targets pending and call `wait_agent` again later.

Classify every `send_input` by delivery time. Guidance for a running agent that can affect its current work—including `FOCUS`, `DELTA`, changed evidence, scope clarification, and correction—uses `ORCHESTRATOR_GUIDANCE:` with `interrupt=true`, so the current task is redirected and the message is handled immediately instead of entering the queue. This does not close the agent. Queue only input deliberately intended after the current task: use `AFTER_CURRENT_TASK:` with explicit `interrupt=false`. When the user explicitly requests a stop or replacement, use `USER_REQUESTED_INTERRUPT:` with `interrupt=true`. Treat all three as delivery envelopes: put exactly one prefix followed by non-empty visible input on the first non-empty line of the sole text carrier, and use `message` or `items`, never both. An `items` envelope has exactly one `type: text` item, though non-text evidence may accompany it; non-text items never carry a `text` field. The guard rejects unclassified input, malformed envelopes, and any prefix/`interrupt` mismatch. After any accepted `send_input`, keep the target pending until a later `wait_agent` call reports that target's current terminal result; never reuse terminal evidence from before the message. Call `close_agent` only after that later explicit terminal entry. The optional guard Hook may add a reminder when the host surfaces a direct non-terminal wait result; it does not observe nested waits reliably or enforce close ordering.

Do not send guidance, interrupt, close, replace, or switch the model of a running agent solely because progress is slow, output is sparse, or a wait timed out. When substantive new guidance can affect current work, deliver it immediately; otherwise keep waiting.
