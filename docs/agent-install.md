...
4. Run `python3 scripts/install.py` as a dry run. Summarize every planned skill link, agent file, hook file, hooks.json change, and any existing drift.
5. Do not overwrite drift or change my existing hook defaults without showing the exact impact and receiving confirmation.
