...
Hooks are behavioral guardrails, not a complete authorization or access-control layer. The tool guard blocks unmarked `send_input` interrupts and accepts orchestration corrections only with one of four closed reason codes, while the main agent still owns evidence, bounded use, delegation, result acceptance, user-directed stopping, and close ordering. A correction interrupt may terminate the agent. Because nested `exec` wait results cannot be reliably associated with later close calls, the guard does not enforce premature `close_agent`; the Skill still requires the main agent to close only after a terminal `wait_agent` result. Runtime validation rejects stale guard registrations for wait or close. Acceptance depends on the main agent reviewing the real diff and validation evidence.

The three orchestration Hooks stay in this repository because they reinforce the Skill's routing and derived-agent identity contracts and share its tests and validator. `subagent_guard.py` intentionally has no terminal marker or close-agent enforcement. Cross-host context injection, memory routing, and closeout behavior belong to their shared runtime instead. A private composition layer may enable both sources on one machine, but it records only paths, deployment mode, and registrations; it does not absorb or redefine either implementation.
