...
Hooks are behavioral guardrails, not a complete authorization or access-control layer. The tool guard requires every subagent input to use one unambiguous delivery envelope: current-task guidance and explicit user interruption pair with `interrupt=true`, while deliberately after-current-task input pairs with explicit `interrupt=false`; the previous closed-reason correction form remains an immediate compatibility alias during upgrades. Unclassified input, mismatched delivery flags, and sidecar prefixes fail closed without tracking cross-call state. An immediate interrupt redirects the current task and does not itself close the agent. The main agent still owns the semantic choice between guidance, later input, and user-directed stopping, plus delegation, result acceptance, and close ordering. Each subagent lifecycle call remains a separate model-visible operation; a host that exposes lifecycle tools only through `functions.exec` makes one call per program and returns its result unchanged. Nested wait Hook delivery is not reliable, so the guard does not inspect outer exec results or enforce premature `close_agent`; after any accepted input, the Skill requires a later explicit terminal entry for that target. Runtime validation rejects stale outer-exec, close, or combined guard registrations. Acceptance depends on the main agent reviewing the real diff and validation evidence.
