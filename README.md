...
- **Simplification review is explicit and read-only.** The optional `simplicity-reviewer` loads one focused method Skill to find removable code, unnecessary abstractions, duplicated defenses, and overbuilt tests without replacing correctness, security, or performance review.
...
- The complete `diagnosing-bugs`, `prototype`, and `simplicity-review` method Skills used by their corresponding Agents.
