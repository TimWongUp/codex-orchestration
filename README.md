...
- Optional hooks that reinforce routing and derived-agent scope, block unmarked interruption of running subagents, and may add stateless advisory context for directly observed non-terminal waits. Close ordering remains a Skill rule owned by the main agent; the Hook does not claim to enforce it.
