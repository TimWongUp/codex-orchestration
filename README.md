...
- **Collaboration has named modes.** `coverage` divides evidence, `panel` compares independent
  ...
  evaluation mode.
- **Review starts with PR work; merge checks reuse it.** `codex-review-gate` runs after creating or updating a PR, when asked to review an existing PR including external contributions, and before an authorized primary-branch merge. Review is independent of CI and runs alongside relevant checks when configured; without CI, use appropriate local validation. Ordinary local completion and branch-only handoff do not trigger it. R0 uses main-agent validation, R1 selects one matching Reviewer, R2 covers broader or uncertain risks, and R3 adds adversarial verification. At merge, reuse evidence that covers the latest candidate and supplement only affected risks; merging and changes to a contributor's branch require their own authority. This policy does not install a GitHub event listener.
...
- **Simplification review is explicit, read-only, and self-contained.** The optional `simplicity-reviewer` finds removable code, unnecessary abstractions, duplicated defenses, and overbuilt tests without replacing correctness, security, or performance review.
